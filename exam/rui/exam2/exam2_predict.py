...
# load models
with open(model_dir + 'cls.model', 'rb') as file:
...
with open(model_dir + 'enc.model', 'rb') as file:
...
X = pd.DataFrame(X, columns=X_columns).astype('float') # set columns order and type same as train set
