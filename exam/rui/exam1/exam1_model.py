...
# # logistic_model = exam1_func.model_func(model_name='lr', model=logistic_model, param_dict=hyperparameters, x_train=train_X, y_train=train_y)
...
# # dtc_model = exam1_func.model_func(model_name='dtc', model=dtc_model, param_dict=hyperparameters, x_train=train_X, y_train=train_y)
# dtc_model.fit(train_X, train_y)
# print("dtc: %s, cost: %.2f" %(dtc_model.score(test_X, test_y), time.time() - startTime))

# xgboost
startTime = time.time()
xgboost_model = xgb.XGBClassifier(n_estimators=1000, eta=0.02, random_state=0, learning_rate=0.01 ,n_jobs=-1) # 'objective': 'multi:softmax' 'num_class': 10,
# max_depth = np.arange(9, 16, 2)
# hyperparameters = dict(max_depth=max_depth)
# xgboost_model = exam1_func.model_func(model_name='xg', model=xgboost_model, param_dict=hyperparameters, x_train=train_X, y_train=train_y)
...
# # hyperparameters = dict(max_depth=max_depth)
# # rf_model = exam1_func.model_func(model_name='rf', model=rf_model, param_dict=hyperparameters, x_train=train_X, y_train=train_y)
...
# svc_model = exam1_func.model_func(model_name='svc', model=svc_model, param_dict=hyperparameters, x_train=train_X, y_train=train_y)
...
# mlp_model = exam1_func.model_func(model_name='mlp', model=mlp_model, param_dict=hyperparameters, x_train=train_X, y_train=train_y)
